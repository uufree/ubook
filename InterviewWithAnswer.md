...
13. TCP是如何确保可靠性的？
    ...
    在数据传输阶段，使用应答-响应、seq number、ack、超时重传的方式，保证数据被正确传输与组装

14. TCP_NODELAY有什么作用？

    当启用TCP_NODELAY时，即使你的数据包很小，也会直接发送；否则等超时（200ms）或者满足MSS（大小1460字节）后发送数据

15. tcp三次握手最后一次失败了会怎么样？此时客户端发数据过去服务端会怎么处理？
...
10. HTTP有什么特点？
    ...
    缺点：明文传输不安全；缺乏身份认证和数据完整性校验；性能问题（长链接的队头阻塞问题）
